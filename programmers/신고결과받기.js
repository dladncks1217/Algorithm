function solution(id_list, report, k) {
  ...
  report.forEach((v, i) => {
    ...
    if (arrestedUser.indexOf(reportedUser) !== -1) {
      // 이게 아니면 정지당한사람이 아님
      let reportedUserIndex = arrestedUser.indexOf(reportedUser); // 정지 유저중 몇번째인지
      if (reportsuccessUser[reportedUserIndex].indexOf(reportUser) === -1) {
        // 아직 신고기록이 없다면
